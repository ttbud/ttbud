...
const ttbudCollisionDetector: CollisionDetection = (args) => {
  ...
  if (rectCollisions.some((col) => col.id === "character-tray")) {
    const relevantContainers = droppableContainers.filter(
      (container) =>
        container.data.current?.origin.containerId === "character-tray"
    );
    const relevantRects = new Map(
    ...
    return closestCenter({
      ...
      droppableRects: relevantRects,
    });
  }

  if (rectCollisions.some((col) => col.id === "floor-tray")) {
    const relevantContainers = droppableContainers.filter(
      (container) => container.data.current?.origin.containerId === "floor-tray"
    );
    const relevantRects = new Map(
      relevantContainers.map((container) => [
        container.id,
