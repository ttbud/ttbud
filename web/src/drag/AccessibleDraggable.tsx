...
const AccessibleDraggable: React.FC<Props> = ({
  descriptor,
  usePortal = false,
  droppableId,
  children,
  ...
  const onClick: MouseEventHandler = useCallback(
    (e) => {
      if (
        (e.buttons !== 0 && e.buttons !== LEFT_MOUSE) ||
      ...
      if (dragState.type === DragStateType.Dragging) {
        dispatch(releaseDrag(descriptor, mousePos));
      } else if (dragState.type === DragStateType.NotDragging) {
        dispatch(
    ...
    [descriptor, dispatch, droppableId, dragState.type]
  );
  ...
  const dragAttrs: DragAttributes = {
    ref,
    style: getStyle(dragState, usePortal),
    ...getHandlers(dragState),
  };

  const element = children(isDragging, dragAttrs);

  const draggable =
    usePortal && isDragging ? createPortal(element, document.body) : element;

  return isDragging ? (
    <>
      <Backdrop key="draggable-backdrop" style={{zIndex: 10_000}} open={true} invisible={true} onClick={onClick}/>
      {draggable}
    </>
  ) : (
    draggable
...
export default AccessibleDraggable;
